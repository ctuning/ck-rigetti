...
#
# Installation script for pyQuil.
#
...
######################################################################################
echo ""
echo "Cleaning up ${EXTRA_PYTHON_SITE} and ${GIT_SRC_DIR} ..."
...
if [ -z "${PACKAGE_VERSION}" ]; then
    ...
    PACKAGE_COMMIT=v${PACKAGE_VERSION}
fi
echo "Cloning ${PACKAGE_COMMIT} @ ${PYTHON_PACKAGE_NAME} into ${GIT_SRC_DIR} ..."
git clone -b $PACKAGE_COMMIT $GIT_SRC_URL $GIT_SRC_DIR

######################################################################################
echo "Installing ${PYTHON_PACKAGE_NAME} to '${PACKAGE_LIB_DIR}' ..."

EXAMPLE_REQ=`ls -1 $GIT_SRC_DIR/examples/requirements.txt 2>/dev/null`

${CK_ENV_COMPILER_PYTHON_FILE} -m pip install $GIT_SRC_DIR ${EXAMPLE_REQ:+"--requirement=${EXAMPLE_REQ}"} --prefix=${EXTRA_PYTHON_SITE} ${PIP_INSTALL_OPTIONS}
