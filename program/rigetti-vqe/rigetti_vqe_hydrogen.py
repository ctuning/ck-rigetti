...
def vqe(q_device, ansatz, hamiltonian, start_params, minimizer_function, minimizer_options, sample_number):
    def expectation_estimation(ab, report):
        ...
        if sample_number is None:
            ...
            result_overlaps = qvm.expectation(state_program,
            ...
            for j, term in enumerate(hamiltonian.terms):
                ...
                if term.id() == "":
                    ...
                    for index, gate in term:
                        ...
                        report_this_iteration[
                            'total_q_seconds_per_c_iteration'] += q_run_seconds  # total_q_time_per_iteration
    ...
    optimizer_output = minimizer_function(expectation_estimation, start_params,
                                          **minimizer_options, args=(report))
...
if __name__ == '__main__':
    ...
    # in this trial, we instead explicitly supply the hamiltonian for "hydrogen, sto-3g"
    ...
    #
    classical_energy = -1.13727017
    start_params = [0.5]
    minimizer_function = minimize
    minimizer_options = {'method': 'Nelder-Mead'}
    sample_number = None

    # ---------------------------------------- run VQE: ----------------------------------------

    (vqe_output, report) = vqe(q_device, hydrogen_ansatz, hamiltonian, start_params, minimizer_function,
                                        minimizer_options, sample_number)
