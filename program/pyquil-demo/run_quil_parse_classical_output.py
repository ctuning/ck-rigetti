...
def main(filepath, classical_register_num):
    ...
    print("---------------------------")
    print("Output: ")
    output_list = qvm.run(program, list(range(classical_register_num)))
    output_dict = { "classical_registers" : output_list[0] }
    print(output_dict)
    with open('qvm_output.json', 'w') as json_file:
        json.dump(output_dict, json_file, sort_keys = True, indent = 4)
